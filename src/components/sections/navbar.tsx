...
const LINKS = [
  ...
  { href: '#humanista', label: 'C-Humanista' },
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <div className="relative rounded-2xl md:rounded-full bg-[#0a1236]/85 backdrop-blur-xl border border-white/15 shadow-[0_8px_32px_rgba(0,0,0,0.45)] px-4 md:px-6 py-2.5 flex items-center justify-between transition-all duration-300">
            ...
            <a href="#inicio" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col">
                <span className="text-white font-heading font-bold text-xs md:text-sm tracking-tight group-hover:text-gold transition-colors">
                  Liceo Juan Antonio Ríos
                </span>
                <span className="text-[10px] text-gold/80 tracking-widest uppercase font-semibold hidden xl:block">
                  Admisión 2027
                </span>
              </div>
            </a>
            ...
            <div className="hidden lg:flex items-center gap-1 xl:gap-1.5">
              {LINKS.map((l) => (
                <a
                  key={l.href}
                  href={l.href}
                  className="text-[11px] xl:text-xs font-medium tracking-wide text-white/80 hover:text-white hover:bg-white/10 px-2.5 py-1.5 rounded-full transition-all duration-200"
                >
                  {l.label}
                </a>
              ))}
